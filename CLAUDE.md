...
- All pages call the same Apps Script web app URL, hardcoded independently in each file's `<script>` config block (`index.html`, `booking.html`, `team.html`, `admin.html`, `scanner.html`). If the Apps Script is ever redeployed under a new URL, it must be updated in all five places — there is no shared config file. `index.html` started calling it only recently (for the theme setting below); before that it was the one page with no backend dependency.
...
| File | Purpose |
|---|---|
| `index.html` | Public landing page. Hero, featured/current event card, services grid, "why us" values, and a WhatsApp-based enquiry form (`sendEnquiry()` builds a `wa.me` link client-side — no backend involved for the form itself). On load it calls the Apps Script `getsettings` action to pick up the remote-controlled theme (see below); if that call fails or is slow, the page just keeps the default Maroon & Gold look. |
| `booking.html` | Customer-facing ticket booking flow: enter name/phone/email, pick adult/kid ticket counts (stepper UI), submit to the Apps Script API to reserve a booking and get a reference number, then a payment screen showing a UPI ID/phone number to pay manually, and a "send screenshot on WhatsApp" button for manual verification. Ends on a "booking received, pending verification" screen. Gated behind the remote `booking_enabled` setting (see below) — currently off since the event has passed; shows a static "booking closed" message when off. |
| `team.html` | Password-gated internal tool for staff to create bookings on behalf of customers calling in by phone. Same ticket-stepper UI as `booking.html` but posts with `internal:true`; on success shows the reference and a pre-filled WhatsApp message (including the event WhatsApp group link) to send the pass directly, skipping the payment-pending step since cash/UPI is collected on the call. Also gated behind a remote `team_enabled` setting, independent of the password gate — currently off. |
| `admin.html` | Password-gated team dashboard: live stats (revenue, tickets sold, adults/kids, pending, to-send, checked-in), tabs for Pending / To Send / All / **Settings**, search, and per-booking actions (Approve, Reject, Send pass on WhatsApp, Mark sent, Call). Auto-refreshes every 30s while the tab is visible. The Settings tab is the non-technical control panel for the remote settings system below — three page on/off toggles and the `index.html` theme swatch picker. |
| `scanner.html` | Camera-based QR check-in scanner (uses the `html5-qrcode` library from a CDN) with a live "guests arrived / total" counter, manual ticket-ID entry fallback (`BPB3-###`), and audio+vibration feedback for valid/invalid scans. Polls scan stats every 60s. Gated behind a remote `scanner_enabled` setting — currently off. |
...
### Remote settings system (page toggles + theme)

Added post-event so the team can flip things on/off or change the homepage mood without asking the developer to edit code and push. Lives partly in Apps Script, partly in each page:

- **Apps Script** (not in this repo) holds a `Settings` sheet tab (auto-created on first use, key/value rows) and two new API actions: `getsettings` (public, no password — pages need to read it before rendering) and `updatesettings` (password-protected like the other admin actions). The known keys are `booking_enabled`, `team_enabled`, `scanner_enabled` (booleans, normalized server-side to the literal strings `'true'`/`'false'` because Google Sheets silently converts text that looks like a boolean into an actual Boolean cell, which would otherwise break a strict string comparison on the client) and `theme` (one of `maroon-gold` / `royal-purple` / `emerald-gold`, validated against an allowlist in `updateSettings`).
- **`booking.html`, `team.html`, `scanner.html`** each fetch `getsettings` before showing anything: a brief "Loading…" screen, then either the real tool or a static "closed" message, decided by their respective `_enabled` flag. If the fetch fails, they fail closed (show the closed message) rather than risk exposing the booking/scan flow.
- **`index.html`** fetches `getsettings` too, but doesn't gate rendering — it repaints in place via a `data-theme` attribute on `<html>` once the fetch resolves, only for the three CSS custom properties that make up the dark background family + red section glow (`--bg-0` through `--bg-6`, `--glow-rgb`). The gold accent is intentionally the same hex values across all three presets; only the theme picker in `admin.html` writes the `theme` key, and only `index.html` reads it — `booking.html`/`team.html`/`scanner.html`/`admin.html` are not themed.
- **`admin.html`**'s Settings tab is the only UI for changing any of this: three toggle switches plus three theme swatches, saved together via one `updatesettings` call.
- The full functional (pre-toggle) code for `booking.html`/`team.html`/`scanner.html` is recoverable from git history or the `v1.0-bpb3-3.0` tag if these ever need to be restored beyond what the toggle can do.
...
- Done: removed the public `/booking` link and reframed `index.html`'s event card as past. Done: `booking.html`/`team.html`/`scanner.html` are now off via the remote settings system above (not deleted — the team can flip them back on from `admin.html` for a future event without a code change).
- Keep `admin.html` reachable (even if unlinked from nav) so the team can still review event data and manage the settings above.
- Still open: evolve `index.html` further into a proper company site — a real photo/video gallery from the event, a "past events" credibility section, etc.
- Do **not** restructure the stack, introduce a build system/framework, or migrate off Google Sheets/Apps Script during this phase unless explicitly told to. (The settings system above is an extension of the existing Apps Script/Sheets pattern, not a departure from it.)
...
- **Settings sheet has no UI validation beyond the allowlist in `updateSettings`**: someone editing the `Settings` sheet tab directly (bypassing `admin.html`) could type a `theme` value outside the three known presets or a non-boolean into an `_enabled` cell; `getSettings()` normalizes booleans defensively but an unrecognized theme string would just fail to match any `[data-theme]` CSS block in `index.html` and silently fall back to the default look, not error.
